...
func (a *lteq) Filter(row map[string]interface{}, filter *Filter) error {
	...
	case operatorTypeNumber:
		...
		if ((*baseline).Cmp(*datum) == -1) || (*baseline).Equals(*datum) {
	...
	case operatorTypeDateime:
		...
		if baseline.Before(*datum) || baseline.Equal(*datum) {
			a.rows = append(a.rows, row)
		}
		break
	default:
...
func (a *lteq) Result() []map[string]interface{} {
	return a.rows
}
